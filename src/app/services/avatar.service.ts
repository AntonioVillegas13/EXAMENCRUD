...
export class AvatarService {
  ...
  updateNote(registro: Registro) {
    const noteDocRef = doc(this.firestore, `citas/${registro.id}`);
    return updateDoc(noteDocRef, { name: registro.name,lastname: registro.lastname,idCard:registro.idCard,schedule:registro.schedule,latitud:registro.latitud,longitud:registro.longitud });
  }
  ...
  getNoteById(id): Observable<Registro> {
    const noteDocRef = doc(this.firestore, `citas/${id}`);
